...
public class InfoSpec2Adapter extends RecyclerView.Adapter<InfoSpec2Adapter.ViewHolder> {
    ...
    public InfoSpec2Adapter(Context context, List<Spec2> list) {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        holder.textView.setText(list.get(position).getSize());

        if (list.get(position).getSize().toString().trim().equals("")){
            holder.textView.setVisibility(View.GONE);
            CommodityInfoActivity.activity.spec_2_text.setVisibility(View.GONE);
        }
